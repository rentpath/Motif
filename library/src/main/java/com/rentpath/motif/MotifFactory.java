...
public class MotifFactory {
    ...
    void onViewCreatedInternal(View view, final Context context, AttributeSet attrs) {
        if (view instanceof TextView) {
            ...
            if (TextUtils.isEmpty(textViewFont)) {
                ...
                if (styleForTextView[1] != -1)
                    textViewFont = MotifUtils.pullFontPathFromTheme(context, styleForTextView[0], styleForTextView[1], mAttributeId);
                else
                    textViewFont = MotifUtils.pullFontPathFromTheme(context, styleForTextView[0], mAttributeId);
            }
            ...
            MotifUtils.applyFontToTextView(context, (TextView) view, MotifConfig.get(), textViewFont, deferred);
        }
        ...
        } else if (MotifConfig.get().isCustomViewTypefaceSupport() && MotifConfig.get().isCustomViewHasTypeface(view)) {
    ...
    private String resolveFontPath(Context context, AttributeSet attrs) {
        ...
        String textViewFont = MotifUtils.pullFontPathFromView(context, attrs, mAttributeId);

        // Try view style attributes
        if (TextUtils.isEmpty(textViewFont)) {
            textViewFont = MotifUtils.pullFontPathFromStyle(context, attrs, mAttributeId);
        }

        // Try View TextAppearance
        if (TextUtils.isEmpty(textViewFont)) {
            textViewFont = MotifUtils.pullFontPathFromTextAppearance(context, attrs, mAttributeId);
        }
